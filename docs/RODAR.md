...
```
  Advocacia IA                                 Agente (ia-juridica)
  ─────────────────────                        ────────────────────
...
O bootstrap inclui `APP_PUBLIC_URL` no CORS. Com front e API em domínios
diferentes, o cookie de sessão precisa de `JWT_COOKIE_DOMAIN` apontando para o
domínio pai dos dois, mais `JWT_COOKIE_SECURE=1` e `JWT_COOKIE_SAMESITE=none` —
sem isso o navegador não manda o cookie e o login "entra e volta". O Jitsi recebe `JITSI_PUBLIC_URL` e `JITSI_ADVERTISE_IPS` na própria
...
## Login: as contas ficam no SQL Server
...
capaz de entrar para consertar.
