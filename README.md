...
Esse é o ponto único de entrada: por padrão ele também sobe a API e o worker do
repositório irmão `ia-juridica` em <http://127.0.0.1:8011>. O Redis do agente é
reconciliado automaticamente e processos que já estiverem saudáveis são reaproveitados,
deixando as execuções seguintes rápidas. `-SemAgente` existe somente para diagnóstico
isolado; sem essa opção, uma falha no agente interrompe a inicialização em vez de entregar
o produto parcialmente funcional.
...
**Tudo exige sessão**, exceto: `/`, `/api/saude`, `/api/config`,
`/api/chamada/config`, `/api/user/authenticate`, `/api/user/logout`, e o prefixo
`/api/portal/` — este protegido pela senha do caso, porque o cliente não tem conta. A lista é de exceções de propósito: rota
...
**Portal do cliente** — sem conta no escritório; protegido pela senha do caso
...
| Método | Rota | Descrição |
|---|---|---|
| `GET` | `/api/saude` · `/api/config` · `/api/eu` | status do modelo, se o login está ligado, quem está autenticado |
...
- **`test_roteiros` zera o `JWT_SECRET` antes de importar `app.main`** — ele chama
  rota protegida sem sessão, e sem isso pararia no primeiro 401.
...
- **A chamada depende do stack do Jitsi no ar.** São quatro contêineres (web,
  prosody, jicofo, videobridge) que ficam de pé entre execuções.
