  ...
  return (
    <div className="max-w-[1320px] mx-auto px-6 pt-5 pb-16 flex flex-col gap-[22px]">
      <div className="flex flex-col gap-3">
        ...
        <p className="text-tinta-2 text-sm m-0 max-w-[76ch]">
          Como o foro decidiu casos comparáveis a este, medido sobre decisões já proferidas.
          Não é previsão de resultado.
        </p>
      </div>

      {carregando && <div className="text-tinta-3 py-10 text-center">Consultando o acervo…</div>}
...
function Recorte({ dados }: { dados: Dados }) {
  ...
  return (
    <Cartao className="flex flex-col gap-[10px]">
      <div>
        <span className="block text-xs uppercase tracking-[0.04em] text-tinta-3">Recorte medido</span>
        <strong className="text-md text-tinta">{dados.scope.label}</strong>
      </div>
      <div className="flex gap-[18px] flex-wrap text-tinta-2 text-sm">
        <span>
      ...
      {(dados.scope.notes ?? []).length > 0 && (
        <ul className="m-0 pl-[18px] text-tinta-2 text-sm flex flex-col gap-1">
...
function LinhaDeComparacao({ linha }: { linha: Proporcao }) {
  ...
  return (
    <li className="flex flex-col gap-1 px-3 py-[10px] bg-papel-2 border border-borda rounded-[8px]">
      ...
      <div className="flex justify-between items-center gap-3 flex-wrap text-tinta-3 text-xs">
        <span>
  ...
  return (
    <ul className="list-none m-0 p-0 flex flex-col gap-[14px]">
      {ranking.slices.map((fatia) => {
        ...
        return (
          <li key={fatia.value} className="flex flex-col gap-[6px]">
            <div className="flex justify-between items-baseline gap-3 text-tinta text-sm [&_span]:text-tinta-3 [&_span]:tabular-nums">
...
function Secao({
  ...
  return (
    <section className="flex flex-col gap-[10px]">
      <h2 className="flex items-center gap-[10px] font-titulo text-lg text-tinta m-0">
        <span className="inline-flex items-center justify-center w-[26px] h-[26px] rounded-full bg-papel-2 border border-borda-forte text-xs text-tinta-2">
          {numero}
        </span>
        {titulo}
      </h2>
      {explicacao && <p className="text-tinta-2 text-sm m-0 max-w-[82ch]">{explicacao}</p>}
