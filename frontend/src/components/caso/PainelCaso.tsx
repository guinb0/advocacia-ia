...
const FILTRO_BASE =
  ...
  "transition-[background-color,border-color,color] duration-[120ms] ease-[ease] hover:bg-papel-3";
const FILTRO_ATIVO =
...
function Campo({
  ...
  return (
    <div className="flex flex-col gap-[2px] min-w-0">
      ...
      {ausente ? (
        <span className="text-sm text-tinta-3 italic font-normal" title={ausente}>
          não registrado
        </span>
      ) : (
        ...
        <div className="flex gap-[6px] items-center flex-wrap bg-papel border border-borda rounded-cartao px-[10px] py-2 shadow-cartao">
          ...
          {PERIODOS.map((opcao) => (
            <button
              key={opcao.valor}
              type="button"
              className={periodo === opcao.valor ? FILTRO_ATIVO : FILTRO_BASE}
          ...
          <span className="text-xs text-tinta-3 mr-1 ml-3">Tipo</span>
          {["documento", "entrevista", "contrato", "agente"].map((tipo) => (
            <button
              key={tipo}
              type="button"
              className={tiposVisiveis.includes(tipo) ? FILTRO_ATIVO : FILTRO_BASE}
                ...
                <div className="flex flex-wrap gap-2">
                  {dados.fatos.por_status.map((linha) => {
                    ...
                    return (
                      <span
                        key={linha.status}
                        className="border border-borda-forte rounded-pill px-[11px] py-[3px] text-xs text-tinta-2"
                      ...
                      >
                        <strong className="text-tinta tabular-nums">{linha.quantidade}</strong>{" "}
                  ...
                  {dados.fatos.apenas_relatados > 0 && (
                    ...
                    >
                      <strong className="text-tinta tabular-nums">{dados.fatos.apenas_relatados}</strong> sem documento que comprove
                    </span>
                  )}
                  {dados.fatos.sem_origem > 0 && (
                    ...
                    >
                      <strong className="text-tinta tabular-nums">{dados.fatos.sem_origem}</strong> sem origem
                    </span>
                  )}
                </div>
      ...
      >
        <div className="grid grid-cols-[repeat(auto-fit,minmax(215px,1fr))] gap-[14px]">
          <div className="bg-papel border border-borda rounded-cartao shadow-cartao px-4 py-[14px] flex flex-col gap-[10px] min-w-0 col-span-2 max-[620px]:col-span-1">
            ...
            <span className="text-xs text-tinta-3 leading-[1.4]" title={saude.base}>
            ...
            <div className="flex flex-col gap-2 mt-1">
              {saude.componentes.map((componente) => (
                ...
                >
                  ...
                  <span className="col-span-full h-[6px] rounded-pill bg-papel-3 overflow-hidden">
                    <span
            ...
            >
              {!ciclo || ciclo.previsto_horas === null || !ciclo.previsto_horas ? (
                ...
                <div className="flex flex-col gap-[14px]">
                  <div>
                    <div className="font-ui text-[1.9rem] font-[650] text-tinta leading-[1.1] flex items-baseline gap-[5px]">
                      {Math.round(
                        ((ciclo.realizado_horas ?? 0) / (ciclo.previsto_horas || 1)) * 100,
                      )}
                      <span className="text-sm font-medium text-tinta-3">% da referência</span>
                    </div>
                    <div className="text-xs text-tinta-3 leading-[1.4]">
              ...
              <div className="flex flex-col gap-3">
                <div>
                  <div className="font-ui text-[1.9rem] font-[650] text-tinta leading-[1.1] flex items-baseline gap-[5px]">
                    ...
                    <span className="text-sm font-medium text-tinta-3">dias</span>
                  </div>
                  <div className="text-xs text-tinta-3 leading-[1.4]">
            ...
            {dados.insights.length === 0 ? (
              ...
              <div className="grid grid-cols-[repeat(auto-fit,minmax(300px,1fr))] gap-3">
                {dados.insights.map((insight, indice) => (
                  <div
                    key={indice}
                    className="flex gap-[10px] px-[14px] py-3 rounded-cartao border border-borda bg-papel items-start"
                    style={{ borderColor: BORDA_DO_TOM[insight.tom], background: FUNDO_DO_TOM[insight.tom] }}
                  >
                    <span
                      className="w-[22px] h-[22px] rounded-full inline-flex items-center justify-center font-bold text-xs flex-none border"
                      style={{
                    ...
                    <div>
                      <div className="text-base text-tinta leading-[1.35]">{insight.texto}</div>
                      <div className="mt-[3px] text-xs text-tinta-3 leading-[1.4]">{insight.base}</div>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <div className={CARTAO}>
          ...
          >
            {eventosFiltrados.length === 0 ? (
              ...
              <>
                <div className="flex flex-col relative pl-[6px]">
                  <span className="absolute left-[142px] top-0 bottom-0 w-[1px] bg-borda max-[640px]:hidden" aria-hidden />
                  {[...eventosFiltrados]
                    .reverse()
                ...
                <table className={TABELA}>
                  <thead>
                    <tr>
                      ...
                      <th className={TH}>Fatos gerados</th>
                    </tr>
                  </thead>
                  <tbody>
            ...
            <div className="grid grid-cols-[repeat(auto-fit,minmax(280px,1fr))] gap-[10px]">
              {dados.ausencias.map((item) => (
                <div key={item.campo} className="border border-dashed border-borda-forte rounded-campo bg-papel-2 px-3 py-[10px]">
                  ...
                  <div className="text-xs text-tinta-3 mt-[2px] leading-[1.45]">{item.motivo}</div>
                </div>
              ))}
            </div>
          </Figura>
...
function LinhaDoEvento({ evento }: { evento: EventoDoCaso }) {
  return (
    <div className="grid grid-cols-[132px_22px_1fr] max-[640px]:grid-cols-[20px_1fr] gap-[10px] items-start py-2 relative">
      ...
      <span
        className="w-5 h-5 rounded-full inline-flex items-center justify-center text-[11px] font-bold border bg-papel [z-index:1]"
